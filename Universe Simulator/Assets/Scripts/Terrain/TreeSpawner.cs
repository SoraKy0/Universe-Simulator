...
{
    public GameObject treePrefab; //the tree object reference
    ...
    public void SpawnTrees()
    {
        if (IsServer)
        {
            ...
            {
                //Randomly generate a points within the square area
                ...
                if (Physics.Raycast(raycastSpawnPoint, Vector3.down, out hit))
                {
                    ...
                    if (hit.point.y >= 0)
                    {
                        ...
                        //rotate the tree randomly to make the trees look natural as if they all faced the same way it will look strange
